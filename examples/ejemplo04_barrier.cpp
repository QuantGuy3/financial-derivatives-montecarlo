...
int main(int argc, char** argv) {
    ...
    const double B   = 120.0;         // up-and-out barrier
    const double r   = 0.05;
    double eps = (argc > 1) ? atof(argv[1]) : 0.05;
    const int    M   = 2;
    const int    L_MAX = 10;

    // Corrección BGK dentro del kernel: barrera ajustada por exp(-BGK_BETA*sigma*sqrt(h))
    ModelVariant  mv = gbm;
    PayoffVariant pv = Barrier{K, B, gbm.sigma, r, gbm.T};

    // ── Referencia (MC 500k, n=64, BGK en el kernel) ─────────────────────────
    ...
    // ── Tabla ─────────────────────────────────────────────────────────────────
    print_table(rows, price_ref, eps, "Ejemplo 04: Up-and-Out Barrier GBM (BGK)");
